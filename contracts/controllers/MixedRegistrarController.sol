    ...
    IRegistrarController
{
    ...
    function initialize(uint256 _minCommitmentAge, uint256 _maxCommitmentAge) public initializer {
        __Ownable_init();
    ...
    function register(Registration.RegisterOrder calldata order) public payable nonReentrant {
        ...
        if (order.params.length > 0) {
            (address voucher, uint256 voucherId) = abi.decode(order.params, (address, uint256));
            require(isVoucher(voucher, voucherId), 'RegistrarController: invalid voucher');

            IVoucher v = IVoucher(voucher);
            cost = v.checkout(voucherId, IVoucher.VoucherEffect.Register, order.registrar, name, price.currency, cost);
            v.burn(msg.sender, voucherId, 1);

            emit VoucherRegistered(order.registrar, keccak256(order.name), name, order.owner, voucher, voucherId);
        }

        IRegistrar registrar = IRegistrar(order.registrar);
        if (cost > 0) {
            if (msg.sender != address(this)) {
                IERC20Upgradeable(price.currency).safeTransferFrom(msg.sender, address(this), cost);
            }
            IERC20Upgradeable(price.currency).safeTransfer(registrar.feeRecipient(), cost);
        }

        (uint256 tokenId, uint256 expires) = registrar.register(name, order.owner, order.duration, order.resolver);

        emit NameRegistered(order.registrar, keccak256(order.name), name, order.owner, tokenId, cost, expires);
    }
    ...
    function renew(
        ...
        if (data.length > 0) {
            (address voucher, uint256 voucherId) = abi.decode(data, (address, uint256));
            require(isVoucher(voucher, voucherId), 'RegistrarController: invalid voucher');

            IVoucher v = IVoucher(voucher);
            cost = v.checkout(voucherId, IVoucher.VoucherEffect.Renewal, registrar, name, price.currency, cost);
            v.burn(msg.sender, voucherId, 1);

            emit VoucherRenewed(registrar, label, name, voucher, voucherId);
        }
        ...
        if (cost > 0) {
            ...
            IERC20Upgradeable(price.currency).safeTransfer(r.feeRecipient(), cost);
        }

        (uint256 tokenId, uint256 expires) = r.renew(uint256(label), duration);

        emit NameRenewed(registrar, label, name, tokenId, cost, expires);
    }
    ...
    function bulkRegister(
        ...
        for (uint256 i = 0; i < orders.length; i++) {
            address(this).call(abi.encodeWithSelector(this.register.selector, orders[i]));
        }

        // return remaining tokens (if any)
        for (uint256 i = 0; i < erc20Details.length; i++) {
            if (IERC20Upgradeable(erc20Details[i].token).balanceOf(address(this)) > 0) {
                erc20Details[i].token.call(
                    abi.encodeWithSignature(
                        'transfer(address,uint256)',
                        msg.sender,
                        IERC20Upgradeable(erc20Details[i].token).balanceOf(address(this))
                    )
                );
            }
        }
        for (uint256 i = 0; i < voucherDetails.length; i++) {
            if (IERC1155Upgradeable(voucherDetails[i].token).balanceOf(address(this), voucherDetails[i].id) > 0) {
                voucherDetails[i].token.call(
                    abi.encodeWithSelector(
                        IERC1155Upgradeable.safeTransferFrom.selector,
                        address(this),
                        msg.sender,
                        voucherDetails[i].id,
                        IERC1155Upgradeable(voucherDetails[i].token).balanceOf(address(this), voucherDetails[i].id),
                        ''
                    )
                );
            }
