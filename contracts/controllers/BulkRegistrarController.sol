...
contract BulkRegistrarController is IBulkRegistrarController {
    ...
    function getController(IRegistrar registrar) internal view returns (IRegistrarController) {
        IResolver r = IResolver(registry.resolver(registrar.baseNode()));
        return
            IRegistrarController(r.interfaceImplementer(registrar.baseNode(), type(IRegistrarController).interfaceId));
    }
    ...
    function nameRecords(
        ...
        IResolver r = IResolver(registry.resolver(IRegistrar(registrar).baseNode()));
    ...
    {
        require(registrars.length == names.length, 'registrars and names lengths do not match');
    ...
    function rentPrice(
        ...
        require(registrars.length == names.length, 'registrars and names lengths do not match');
        require(names.length == durations.length, 'names and durations lengths do not match');

        RentPrice[] memory prices = new RentPrice[](names.length);
        for (uint256 i = 0; i < names.length; i++) {
            IRegistrarController controller = getController(IRegistrar(registrars[i]));
            IPriceOracle.Price memory price = controller.rentPrice(registrars[i], names[i], durations[i]);
    ...
    function bulkRenew(
        ...
        require(registrars.length == names.length && names.length == durations.length, 'arrays lengths do not match');
        ...
        for (uint256 i = 0; i < names.length; i++) {
            ...
            if (price.currency == NATIVE_TOKEN_ADDRESS) {
                controller.renew{ value: price.base + price.premium }(registrars[i], names[i], durations[i]);
            } else {
    ...
    function supportsInterface(bytes4 interfaceID) external pure returns (bool) {
        return interfaceID == type(IERC165).interfaceId || interfaceID == type(IBulkRegistrarController).interfaceId;
    }
}
