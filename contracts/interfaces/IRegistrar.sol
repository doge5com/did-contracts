...
interface IRegistrar is IControllable {
    event NameRegistered(uint256 indexed tokenId, uint256 indexed labelId, address indexed owner, uint256 expires);
    event NameRenewed(uint256 indexed tokenId, uint256 indexed labelId, uint256 expires);
    ...
    // Extend a name.
    function renew(uint256 labelId, uint256 duration) external returns (uint256 tokenId, uint256 expires);
